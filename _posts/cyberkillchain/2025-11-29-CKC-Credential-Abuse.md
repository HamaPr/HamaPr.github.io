...
본 공격 시나리오(Operation Name: **Credential Abuse: Identity Breach**)는 `CKCProject` 인프라를 대상으로 **신원 기반 공격(Identity-Based Attack)**에 대한 방어 태세를 점검하기 위해 수행되었다. External RCE Chain 시나리오가 외부 애플리케이션의 기술적 취약점(RCE)을 다루었다면, Credential Abuse 시나리오는 **"잘못 관리된 인프라 코드(IaC)에서 유출된 단일 자격 증명(SSH Key)이 어떻게 전체 클라우드 환경의 붕괴로 이어지는가"**에 초점을 맞춘다.

본 시나리오는 Terraform 코드(`main.tf`)에 의해 생성되어 로컬 파일(`leaked_id_rsa`)로 저장된 SSH Private Key가 개발자의 실수로 공개 Git 저장소에 커밋되었다는 가정에서 출발한다. 공격자는 이 '만능 열쇠'를 사용하여 정상적인 관리 경로인 **Jumpbox VM**으로 침투하고, 내부 시스템에 만연한 구조적인 설정 오류와 신뢰 관계를 악용하여 최종 목표까지 도달한다.
...
3.  **내부 신뢰 모델 분석:** 단일 SSH 키가 내부망 전체(Bastion, Web, DB)에 재사용되고 있을 때, 공격자가 어떻게 최소한의 흔적만으로 내부망을 수평 이동(Lateral Movement)할 수 있는지 파악한다.

#### **0.3. 타겟 시스템 아키텍처 및 공격 경로 (Architecture & Attack Vector)**

공격 경로는 Load Balancer를 거치는 정규 서비스 경로가 아닌, 관리자 전용 **Jumpbox VM**(Terraform 리소스 `azurerm_linux_virtual_machine.jumpbox_vm`)을 통해 Private Subnet의 핵심 자산으로 직행하는 우회 경로를 따른다.

*   **Public IP (Jumpbox):** `20.214.x.x` (Terraform `jumpbox_pip` Output)
*   **Public Subnet (Bastion Tier):** `10.42.1.0/24` (초기 침투 지점)
*   **DMZ Subnet (Web Tier):** `10.42.2.0/24`
*   **Private Subnet (DB Tier):** `10.42.3.0/24`
...
| **3. Docker 그룹을 통한 권한 상승** | 서버 빌드 표준(Hardening) 미준수 | **[운영/규정준수]** 공격자에게 즉각적인 Root 권한을 허용하여 데이터 파괴, 랜섬웨어 등 심각한 사고 유발 가능 |
...
| **Lateral Movement** | [T1021.004] Remote Services: SSH | 자격 증명 재사용 (Credential Re-use) |

---

### **1. 정찰 (Reconnaissance)**

**목표:** 대상 조직의 공개된 디지털 자산, 특히 인프라 코드(IaC) 저장소를 대상으로 수동적 정찰(Passive Reconnaissance)을 수행하여, 네트워크 경계를 우회할 수 있는 민감 정보(Secrets), 특히 인증 자격 증명을 수집한다.
...
3.  **키 무결성 및 핑거프린트 확인:**
    ...
    ```
    **[Critical Finding]** 핑거프린트 확인 결과 **4096-bit RSA 키**임을 확인했다. 키에 주석(comment)이 포함되어 있지 않아 직접적인 사용자 식별은 불가능했지만, Terraform 코드의 `admin_ssh_key` 블록과 연관된 리소스 분석을 통해 **`azureadmin`** 계정에서 사용됨을 유추할 수 있었다.
...
*   **Credential:** `leaked_id_rsa` (암호 없는 4096-bit RSA 키)
*   **User Account:** `azureadmin`
*   **Target System:** 키 생성 관련 Terraform 코드(`resource "azurerm_linux_virtual_machine" "jumpbox_vm"`)와 커밋 메시지("bastion setup")를 통해, 이 키의 주 사용처가 **Jumpbox VM**임을 확신했다.
*   **Entry Point:** Terraform 출력값(`output "ATTACK_TARGETS"`)을 통해 Jumpbox VM의 Public IP를 확보하고, 해당 IP의 22번 포트(SSH)를 최종 진입점으로 결정했다.
...
Host target-bastion
    ...
    User azureadmin
    IdentityFile ~/leaked_id_rsa      # Weaponized Key
...
**[Delivery Process]**
1.  **Handshake:** 공격자 PC가 Jumpbox VM의 Public IP(4.217.x.x) 22번 포트로 TCP 연결 시도.
2.  **Authentication:** SSH 클라이언트가 `leaked_id_rsa` 비공개 키를 사용하여 인증 응답 생성 및 서버에 전송.
3.  **Verification:** Jumpbox VM의 SSH 데몬은 `/home/azureadmin/.ssh/authorized_keys`에 저장된 공개 키와 대조하여 요청을 검증.
4.  **Success:** 키가 유효하므로, SSH 데몬은 접근을 허용하고 셸(Shell)을 생성한다.

![Bastion 접속 성공](/assets/images/ckc/b/b.3.1.bastion-login.png)

이 모든 과정은 암호화된 SSH 터널 내부에서 일어나며, 외부에서는 정상적인 관리자 로그인과 전혀 구별할 수 없다.

#### **3.2. 탐지 및 방어 전략 (Detection & Mitigation)**
...
**목표:** 성공적으로 인증을 통과하여 획득한 초기 셸(Shell) 환경을 정밀 분석하고, 시스템 내에 존재하는 추가적인 취약점, 특히 **권한 상승(Privilege Escalation) 경로**를 식별하여 시스템의 완전한 제어권을 확보할 준비를 마친다.
...
**[발견 사항]**
`id` 명령어 실행 결과, `azureadmin` 계정이 **`docker` 그룹 (GID 122)**에 속해 있음이 확인되었다. 이는 관리 편의를 위해 설정된 보안 허점이다.

*   **Risk:** `docker` 그룹 사용자는 패스워드 없이 Docker 소켓(`/var/run/docker.sock`)에 직접 명령을 내릴 수 있다. Docker 데몬은 `root` 권한으로 실행되므로, 이는 곷바로 **호스트(Host)의 `root` 권한을 획득할 수 있는 직접적인 경로**를 의미한다. 이 경로는 `sudo` 비밀번호를 몰라도 악용이 가능하다.

#### **4.3. 내부망 확산 경로 분석: SSH 키 재사용**

**[분석]**
공격자는 "하나의 키를 유출한 개발자는 다른 곳에도 같은 키를 사용했을 것"이라는 가설을 세웠다. Terraform 코드(`main.tf`)를 보면, `jumpbox_vm`, `web_vmss`, `db_vm` 모두가 **동일한 공개 키(`tls_private_key.attacker_key.public_key_openssh`)**를 `admin_ssh_key`로 사용하도록 설정되어 있다. 공격자는 이 가설을 Jumpbox VM 내부에서 직접 검증했다.
...
#### **4.4. 탐지 및 방어 전략 (Detection & Mitigation)**
...
*   **SSH 키 재사용 방지:**
    ...
    *   **인증서 기반 SSH:** SSH 키 대신 유효 기간이 짧은 SSH 인증서(Certificate)를 발급하여 사용하는 방식으로 전환하면, 키 유출 시의 위험을 최소화할 수 있다.

---

### **5. 설치 (Installation)**

**목표:** 초기 접근에 사용된 SSH 키가 교체되거나 비활성화될 경우에 대비하여, 공격자가 언제든지 다시 시스템에 접근할 수 있도록 **독립적이고 영구적인 백도어(Backdoor)**를 설치한다. 이 단계의 핵심은 **탐지 회피(Defense Evasion)**와 **생존성(Survivability)**이다.
...
#### **5.2. C2 임플란트 전송 및 은닉 (Ingress Tool Transfer)**
...
`systemd`에 의해 실행된 `kworker_sys` 임플란트는 즉시 공격자의 C2 서버로 **아웃바운드 콜백(Outbound Callback)**을 시도했다. 이 통신은 TCP 443 포트를 사용하며, 모든 데이터는 mTLS로 암호화되어 있어 네트워크 보안 장비는 이를 일반적인 HTTPS 트래픽으로 인식하고 통과시킨다.
...
*   **네트워크 행위 분석 (NBA):**
    *   내부 서버가 외부의 특정 IP와 장시간 지속적으로 연결을 유지하거나, 주기적으로 데이터를 주고받는 '비콘(Beacon)' 행위를 탐지해야 한다.
*   **프로세스 트리 분석:**
...
**목표:** 완전히 장악한 Jumpbox VM을 **내부망 공격의 전초기지(Pivot Point)**로 삼아, SSH 키 재사용 취약점을 이용해 최종 목표인 **데이터베이스 서버를 장악하고 민감 정보를 유출**한다.
...
#### **7.1. 1단계: 수평 이동 (Lateral Movement via Credential Re-use)**
...
#### **7.2. 2단계: 핵심 데이터 수집 (Collection on DB Server)**
...
#### **7.3. 3단계: 다단 유출 (Multi-hop Exfiltration)**

DB 서버는 인터넷으로 직접 나가는 아웃바운드 경로가 없다. 따라서 공격자는 데이터를 **Jumpbox VM으로 1차 전송**한 뒤, Jumpbox VM에 설치된 C2 채널을 통해 **외부로 최종 반출**하는 **다단 유출(Multi-hop Exfiltration)** 전략을 사용했다.
...
#### **8.1. 종합 위협 분석: "신원이 곧 경계다 (Identity is the Perimeter)"의 붕괴**
...
1.  **IaC 보안 관리 실패:** Terraform 코드 관리 프로세스에 **비밀정보 스캐닝(Secret Scanning)**이 부재하여, 생성된 SSH 키가 아무런 통제 없이 외부로 유출될 수 있는 환경이었다.
2.  **자격 증명 관리 실패 (Credential Mismanagement):**
...
| **Action** | Lateral Movement | 서버 간 동일 SSH 키 재사용 (Credential Reuse). |

#### **8.3. 보안 강화 로드맵 (Strategic Roadmap)**

| 단계 | 목표 | Action Items |
| :--- | :--- | :--- |
| **Phase 1: 긴급 조치 (24h)** | **현재 위협 제거 및 출혈 차단** | 1. **자격 증명 전면 교체:** 유출된 키 쌍을 즉시 폐기하고, **모든 서버의 SSH 키를 고유한 키로 재발급**. (`for server in $(az vm list...); do az vm user update...; done`) <br> 2. **권한 축소:** 모든 사용자 계정에서 `docker` 그룹 권한 즉시 제거. (`gpasswd -d azureadmin docker`) <br> 3. **Jumpbox 접근 통제:** SSH Key 인증 외 **Azure AD(Entra ID) 기반 MFA** 또는 **NSG 소스 IP 제한** 긴급 적용. |
| **Phase 2: 구조 개선 (1 Month)** | **공격 난이도 상향 및 내부 확산 방지** | 1. **Secret Scanning 자동화:** **GitHub Actions**에 `TruffleHog` 스캔을 통합하여, Pull Request 생성 시 민감 정보가 포함되면 **Merge를 자동 차단(Gatekeeping)**. <br> 2. **JIT(Just-In-Time) VM Access 도입:** **Microsoft Defender for Cloud의 JIT 기능**을 활성화하여 상시 열려있는 SSH 포트를 제거하고, 감사된 요청에 대해서만 한시적으로 접근 허용. <br> 3. **SSH 인증서 기반 접근:** 단기 수명(Short-lived)의 **SSH 인증서**를 발급하는 중앙 CA(예: HashiCorp Vault, Teleport)를 구축하여 키 관리 부담 제거. |
| **Phase 3: 체질 개선 (6 Months)** | **Zero Trust 아키텍처로의 전환** | 1. **PIM(Privileged Identity Management):** Azure PIM을 도입하여 상시 관리자 권한을 제거하고, 작업 시 승인/감사 절차를 거쳐 권한을 부여받고 자동 회수되는 체계 구축. <br> 2. **코드형 보안(Security as Code):** CI/CD 파이프라인에 `Checkov`, `tfsec`과 같은 **IaC 스캔 도구**를 통합하여, "docker 그룹에 사용자 추가"와 같은 위험한 설정이 포함된 Terraform 코드가 배포되기 전에 차단. <br> 3. **내부망 가시성 확보:** **Azure Network Watcher**와 **Sentinel**을 연동하여 내부 서브넷 간의 트래픽(East-West)을 분석하고, 비정상적인 프로토콜/볼륨 통신에 대한 자동 경보(Alert) 체계 수립. |

#### **8.4. 기술적 세부 권고 (Technical Deep Dive)**
