...
보안 업계에서 방어를 잘하려면 공격을 이해해야 한다는 말이 있다. 하지만 대부분의 보안 실습은 단편적인 취약점 하나를 exploitation하는 수준에 그친다. 실제 APT 공격은 정찰부터 목적 달성까지 연쇄적인 단계를 거치며 진행된다.
...
flowchart TB
    ...
    classDef c2 fill:#4B0082,stroke:#9400D3,stroke-width:2px,color:white;
    classDef fleet fill:#006400,stroke:#00ff00,stroke-width:2px,color:white;

    subgraph Local ["공격자 로컬"]
        Kali["Kali Linux VM"]:::attacker
    end

    subgraph Azure ["Azure Cloud"]
        C2["C2 Server<br/>hamap.shop"]:::c2
        RD["Redirector<br/>redirect.hamap.shop"]:::c2
        Fleet["Scanner Fleet<br/>(Reconnaissance)"]:::fleet
    end
...
C2 Server는 공격 작전의 중심이다. 페이로드 호스팅부터 세션 관리까지 모든 작업이 이곳에서 이루어진다.
...
```
피해자 → redirect.hamap.shop → [Nginx 필터] → C2 Server
                                    ↓
...
![Redirector 필터링 검증](/assets/images/ckc/ckc.a.3.2.redirector-log.png)
*(그림: User-Agent 기반 필터링 동작 검증. 비정상 UA는 302 Redirect, 정상 UA는 200 OK)*
...
```mermaid
flowchart TB
    classDef public fill:#4169E1,stroke:#fff,color:white;
    classDef dmz fill:#228B22,stroke:#fff,color:white;
    classDef private fill:#8B0000,stroke:#fff,color:white;
    classDef lb fill:#333,stroke:#fff,color:white;

    Internet((("Internet")))
    
    subgraph Azure ["Azure VNet 10.42.0.0/16"]
        subgraph Public ["Public Subnet 10.42.1.0/24"]
            Bastion["Bastion VM"]:::public
        end
        
        subgraph DMZ ["DMZ Subnet 10.42.2.0/24"]
            LB["Load Balancer"]:::lb
            Web["Web VMSS"]:::dmz
        end
        
        subgraph Private ["Private Subnet 10.42.3.0/24"]
            DB["DB Server"]:::private
        end
    end
...
| Private | 10.42.3.0/24 | 데이터 저장 | DB Server + Redis |

### 4.3 취약 서비스 매핑

배포된 취약 서비스와 적용된 표준의 매핑이다.

| 포트/경로 | 취약점 | OWASP 2025 | 주통기 2025 | 시나리오 |
|---------|-------|------------|------------|--------|
| 8081 | CVE-2017-5638 Struts2 RCE | A03 | 웹-01 | External RCE Chain |
| 8082 | CVE-2017-12149 JBoss RCE | A08 | 웹-01 | Legacy Service Pivot |
| 8083 | CVE-2021-44228 Log4Shell | A03 | 웹-01 | External RCE Chain |
| 5000 | Command Injection | A05 | 웹-01 | External RCE Chain |
| 6379 | Redis No Auth | A02 | D-19 | Legacy Service Pivot |
| 3306 | MySQL 약한 자격 증명 | A07 | D-01 | External RCE / Legacy Pivot |
| 22 | SSH 키 유출 + Docker PrivEsc | A02 | U-53 | Credential Abuse |
| /login/ | SQL Injection | A05 | 웹-01 | External RCE Chain |
| /search.php | 반사형 XSS | A05 | 웹-02 | External RCE Chain |
| /guestbook.php | 저장형 XSS | A05 | 웹-02 | External RCE Chain |
| /xss_dom.html | DOM XSS | A05 | 웹-02 | External RCE Chain |
| /admin/settings.php | CSRF | A01 | 웹-03 | External RCE Chain |
| /api/fetch.php | SSRF | A01 | 웹-21 | External RCE Chain |
| /backup/*.sql | 정보 노출 | A02 | 웹-20 | External RCE Chain |
...
| masscan | 대규모 포트 스캔 | nmap보다 빠른 속도 |
| nuclei | 취약점 템플릿 스캔 | 자동화된 CVE 탐지 |
...
| 시나리오 | 제목 | 공격 벡터 | 최종 목표 |
|--------|-----|---------|---------|
| [CKC-External-RCE-Chain](./2025-11-28-CKC-External-RCE-Chain.html) | 외부 RCE 연쇄 공격 | Struts2 / Log4Shell | DB 데이터 유출 |
| [CKC-Credential-Abuse](./2025-11-29-CKC-Credential-Abuse.html) | 자격증명 탈취 및 컨테이너 탈출 | 유출 키 + Docker PrivEsc | 내부망 전체 장악 |
| [CKC-Legacy-Service-Pivot](./2025-11-30-CKC-Legacy-Service-Pivot.html) | 레거시 서비스 피봇 공격 | JBoss RCE → Redis 악용 | DB 서버 Root 탈취 |
