...
```mermaid
flowchart LR
    Operator["Attacker PC"] -- "SSH Control" --> C2["Attacker C2 Server (Azure VM)"]
    C2 -- "1. Attack Traffic (hamap.shop)" --> LB["Target Load Balancer"]
    LB -- "2. Forwarding" --> WebVM["Target Web Server (Private)"]
    WebVM -- "3. Reverse Shell (Outbound)" --> C2
```
...
```mermaid
flowchart LR
    Operator["공격자 PC (Kali)"] -- "SSH (Encrypted)" --> C2["Attacker C2 VM (Azure)"]
    
    subgraph "Attacker Infrastructure (hamap.shop)"
        direction TB
        C2_Web["Nginx Web Server"]
        C2_LDAP["Marshalsec LDAP"]
        C2_List["Netcat Listener"]
    end
    
    C2_Web -- "HTTPS (Payload Delivery)" --> Target["Target Web Server"]
    C2_LDAP -- "LDAP Redirect" --> Target
    Target -- "Reverse Shell (TCP)" --> C2_List
```
...
> **[스크린샷 2: Gobuster 실행 결과 - /test.php, /upload, /login 등 주요 공격 벡터 발견]**
...
> **[스크린샷 1: msfvenom으로 genesis.elf 생성 후 scp로 C2 서버에 전송하는 화면]**
...
> **[스크린샷 2: struts2_exploit.py 스크립트 코드 일부 또는 준비 화면]**
...
> **[스크린샷 3: C2 서버에서 Marshalsec(LDAP)과 Python HTTP 서버를 구동하고 대기하는 화면]**
...
> **[스크린샷 3: struts2_exploit.py 실행 후 Timeout 메시지가 출력된 터미널 화면]**
...
> **[스크린샷 4: Log4Shell 공격을 위해 curl 명령어를 입력하는 화면]**
> **[스크린샷 5: C2 서버에서 확인된 Marshalsec LDAP 로그 및 Python HTTP 서버 접속 로그]**
...
> **[스크린샷 6: 공격자 VM에서 nc 리스너에 연결이 수립된 화면 (Connection received...)]**
...
> **[스크린샷 7: 셸에서 id 및 hostname 명령어를 입력하여 root 권한과 컨테이너 ID를 확인한 화면]**
...
> **[스크린샷 8: Python 스크립트로 내부 DB 서버 접속 성공(Connection OK) 화면]**
...
> **[스크린샷 9: mysqldump 수행 및 C2 서버로 파일 전송 완료 화면]**
