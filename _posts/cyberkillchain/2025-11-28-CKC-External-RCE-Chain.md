...
본 공격 시나리오(Operation Name: **External RCE Chain: Perimeter Breach**)는 외부의 위협 행위자가 인터넷에 공개된 기업의 웹 애플리케이션을 통해 내부 시스템으로 침투하는 가장 전형적이면서도 파괴적인 공격을 재현한다.

본 작전의 핵심은 **패치되지 않은 레거시 애플리케이션(Struts2, Log4j 등)**이 기업의 다층 방어 아키텍처에 어떤 균열을 만들 수 있는지, 그리고 **최소 권한 원칙(Least Privilege)**을 위반한 컨테이너 설정이 어떻게 공격의 피해를 증폭시키는지를 실증하는 데 있다.

#### 0.2. 수행 목적 (Objectives)

본 시나리오는 다음의 핵심 보안 가설을 검증한다.

1.  **경계 방어 유효성 검증:** Load Balancer와 NSG로 구성된 외부 경계 방어 체계가 알려진 치명적 취약점(CVE) 공격 트래픽을 탐지 및 차단할 수 있는가?
...
`CKCProject` 인프라는 Load Balancer를 통해 외부 트래픽을 DMZ Subnet의 웹 서버(VMSS)로 전달하며, 이 VM 위에서 다수의 취약한 Docker 컨테이너가 서비스되고 있다.

*   **Public IP (LB):** `20.214.x.x` (Terraform `lb_pip` Output)
*   **Web Tier (DMZ Subnet):** `10.42.2.0/24` (VMSS Host)
    *   **Docker Containers:** `struts2-rce`, `log4shell-app`, `elasticsearch-rce` 등
*   **DB Tier (Private Subnet):** `10.42.3.0/24` (최종 공격 목표)
...
| **2. 컨테이너의 `root` 권한 실행으로 인한 피해 확산** | 컨테이너 빌드/배포 시 최소 권한 원칙(Least Privilege) 위반 | **[운영]** 단순 '웹 애플리케이션 침해'가 즉시 **'서버 인프라 전체 장악'**으로 격상됨. 공격자의 잠복 및 내부 확산 활동을 위한 완벽한 교두보 제공. |
| **3. 내부망 신뢰 기반의 평면적 네트워크 구조** | 내부 통신(East-West)에 대한 Zero Trust 원칙 부재 및 네트워크 세그멘테이션 미흡 | **[데이터/신뢰도]** 경계가 뚫린 후, 내부 핵심 자산(DB)까지 도달하는 데 아무런 장애물이 없어 고객 정보 등 민감 데이터 유출로 직결. 기업 신뢰도에 치명타. |

#### 0.5. 공격 도구 및 TTPs (Tools, Tactics, and Procedures)

| Kill Chain 단계 | MITRE ATT&CK 기법 | 사용 도구 / 방법 |
| :--- | :--- | :--- |
| **1. 정찰 (Reconnaissance)** | [T1595.001] Active Scanning | `nmap`, `gobuster`, Custom Fleet |
| **2. 무기화 (Weaponization)** | [T1587.001] Develop Capabilities | `Sliver C2`, Python Script, `Marshalsec` |
| **3. 유포 (Delivery)** | [T1190] Exploit Public-Facing App | `curl`, OGNL/JNDI Payloads |
| **4. 악용 (Exploitation)** | [T1059.004] Unix Shell | Docker 컨테이너 내 RCE 실행 |
| **5. 설치 (Installation)** | [T1543.002] Systemd Service | `wget`, Systemd 서비스 등록 |
| **6. C2 (Command & Control)** | [T1573.002] Encrypted Channel | Sliver mTLS Beacon |
| **7. 목적 달성 (Actions on Objectives)** | [T1041] Exfiltration Over C2 | `mysql`, `mysqldump`, Sliver Upload |

---

### 1. 정찰 (Reconnaissance)

**목표:** 타겟의 유일한 진입점인 Load Balancer의 공개 IP를 대상으로 능동적 스캔을 수행하여, 외부에 노출된 서비스들의 종류와 버전을 식별하고 즉시 악용 가능한 공격 표면을 매핑한다.

*   **[T1595.001] Active Scanning:** IP 블록 및 취약점 스캔.
*   **[T1592] Gather Victim Host Information:** 소프트웨어 버전 및 구성 정보 수집.

#### 1.1. 분산 포트 스캐닝 (Distributed Port Scanning)

실제 공격에서 단일 IP로 스캔하면 WAF/IPS에 의해 즉시 차단될 위험이 높다. 이를 회피하기 위해 [프로젝트 개요](./2025-11-20-CKC-Project-Overview.html)에서 구축한 **Custom Distributed Scanner Fleet**을 활용하여 분산 스캐닝을 수행했다.
...
#### 1.2. 서비스 식별 및 핑거프린팅 (Service Identification & Fingerprinting)
...
| **P2 (High)** | **8082** | **jboss-rce** | `jboss/wildfly:10.1.0.Final` 이미지 기반. Java 역직렬화(Deserialization) RCE 취약점(**CVE-2017-12149**)에 노출. |
...
#### 1.3. 웹 경로 분석 및 추가 공격 벡터 식별 (Web Path Analysis)
...
#### 1.4. 최종 공격 벡터 선정 및 전략 수립 (Attack Vector Selection)
...
#### 2.1. C2 페이로드 제작 및 스테이징 (Payload Creation & Staging)
...
#### 2.2. 익스플로잇 체인 구축 (Exploit Chain Construction)
...
*   **Command Injection:** `wget`을 통해 C2 페이로드를 다운로드하고 실행하는 명령어를 주입하여, 리버스 셸 연결이 자동으로 수립되도록 페이로드를 구성했다.
...
#### 3.1. Apache Struts2 악성 OGNL 구문 전송 (Malicious OGNL Injection)
...
#### 3.2. 유포 성공 검증 (Delivery Verification)
...
*   **아웃바운드 통제 (Egress Filtering):**
    *   **문제점:** 공격 성공의 가장 결정적인 요인은 **아웃바운드(Outbound) 트래픽에 대한 통제 부재**였다. 컨테이너 내부에서 인터넷 상의 임의의 IP/포트로 자유롭게 접속 가능한 환경이 리버스 셸 연결의 근본 원인이다.
    *   **해결책:** Azure Firewall 또는 NSG를 활용하여, OS 업데이트 저장소나 허가된 API 등 **업무에 필수적인 도메인/IP 외의 모든 아웃바운드 트래픽을 차단(Default Deny)**해야 한다.
...
#### 4.1. RCE 실행 및 리버스 셸 세션 수립 (RCE & Reverse Shell)

3단계에서 주입된 명령어가 타겟 애플리케이션에 의해 처리되는 순간, `wget`으로 다운로드된 Sliver 비콘 임플란트가 실행되어 공격자의 C2 서버(`hamap.shop`)로 **역접속(Reverse Connection)**을 시도했다. 공격자 C2 서버의 Sliver mTLS 리스너에 암호화된 연결이 수립되며, 완전한 원격 제어권이 확보되었다.

```bash
# Sliver C2 Console (Attacker Side)
...
#### 4.2. 초기 권한 및 환경 분석 (Initial Context Analysis)
...
#### 5.1. 컨테이너에서 호스트로의 페이로드 전이 및 은닉 (Payload Transfer & Hiding)
...
**분석:**
*   **Masquerading:** 서비스 이름(`azure-network-observer`)과 설명(`Azure Network Performance Observer`)을 마치 정상적인 Azure 관리 에이전트인 것처럼 위장했다.
*   **Persistence:** `Restart=always`와 `enable` 설정을 통해, 프로세스가 강제 종료되거나 시스템이 재부팅되더라도 서비스가 자동으로 재시작되어 C2 연결이 복구된다.
...
#### 6.3. 연결 상태 검증 (Connection Verification)
...
*   **JA3/JA3S Fingerprinting:**
    *   일반적인 웹 브라우저나 클라이언트가 아닌, Sliver/Go 언어 특유의 TLS 핸드셰이크 지문(Fingerprint)을 식별하여 차단해야 한다.
...
```bash
# Sliver C2 Session (Web Host Root Shell)
...
#### 7.2. 2단계: 자격 증명 탈취 및 수평 이동 (Credential Dump & Lateral Movement)
...
2.  **수평 이동:**
    ...
    mysql>
    ```
    **결과:** DMZ(웹) 구간과 내부망(DB) 구간 사이의 **네트워크 분리(Segmentation)가 미흡**하여, 웹 서버에서 DB 서버로의 직접 접속이 성공했다.

#### 7.3. 3단계: 핵심 데이터 수집 및 유출 (Data Exfiltration)

공격자는 DB 서버의 제어권을 확보하고, `customer_db`를 최종 목표로 삼았다.

1.  **데이터 수집 및 압축 (Staging):**
    `mysqldump`를 이용해 고객 테이블 전체를 덤프하고, `tar`로 압축하여 파일 크기를 줄이고 탐지를 회피했다. 파일명은 정상적인 백업 파일처럼 `db_backup_20251125.tar.gz`로 위장했다.

    ```bash
    # On DB Server (via mysql client from Web Host)
    mysql> exit;
...
*   **네트워크 세그멘테이션 (Network Segmentation):**
...
#### 8.1. 종합 위협 분석: 단일 실패 지점의 연쇄 붕괴 (Threat Analysis)

본 공격 시나리오는 단 하나의 방치된 **공급망 취약점(Log4j/Struts2)**이 어떻게 기업의 방어 체계 전체를 무너뜨리는지를 명확히 보여주었다. 각 방어 계층은 독립적으로 동작했을 뿐, 연계된 공격 흐름을 막아내지 못했으며, 이는 다음과 같은 구조적 실패에 기인한다.
...
#### 8.2. Kill Chain 단계별 방어 실패 분석 (Defense Failure Analysis)

| 단계 | 공격 기법 | 방어 실패 원인 |
| :--- | :--- | :--- |
| **Recon** | Port Scanning | 로드 밸런서가 모든 포트 스캔 트래픽을 허용함. |
| **Delivery** | OGNL/JNDI Injection | WAF 부재로 악성 페이로드 차단 실패. |
| **Exploitation** | RCE & Reverse Shell | 컨테이너 런타임 보안 부재로 셸 실행 탐지 실패. |
...
| 단계 | 목표 | Action Items |
| :--- | :--- | :--- |
| **Phase 1: 긴급 조치 (24h)** | **현재 위협 제거 및 출혈 차단** | 1. **긴급 패치:** Log4j/Struts2 라이브러리 즉시 업데이트. <br> 2. **C2 차단:** 방화벽에서 악성 도메인(`hamap.shop`) 및 IP 차단. <br> 3. **아웃바운드 통제:** 웹/DB 서버의 인터넷 트래픽을 Default Deny로 변경하고 필수 도메인만 허용. |
| **Phase 2: 구조 개선 (1 Month)** | **공격 난이도 상향 및 내부 확산 방지** | 1. **컨테이너 하드닝:** 모든 Dockerfile에 `USER 1000` 등 non-root 사용자 지정. 컨테이너 파일 시스템 `Read-Only` 설정. <br> 2. **Secrets Management:** **Azure Key Vault**를 도입하여 소스 코드 내 모든 비밀번호 제거. <br> 3. **WAF 적용:** OGNL/JNDI Injection 패턴 차단 룰셋 적용. |
| **Phase 3: 체질 개선 (6 Months)** | **지속 가능한 선제적 방어 체계 구축** | 1. **DevSecOps 파이플라인:** CI/CD에 **SCA(Software Composition Analysis)** 도구를 통합하여 빌드 시점에 취약한 라이브러리 자동 차단. <br> 2. **Zero Trust Network:** 내부 통신에도 마이크로 세그멘테이션을 적용하고, 워크로드 단위 인증 강화. <br> 3. **EDR/NDR 고도화:** "Java 프로세스가 Shell 실행" 등 행위 기반 탐지 룰셋을 개발하고 관제. |

#### 8.4. 기술적 세부 권고 (Technical Recommendations)
...
External RCE Chain 시나리오는 현대 클라우드 환경에서도 **기본 보안 수칙(패치 관리, 권한 관리, 네트워크 격리)**이 무너지면, 정교한 제로데이 공격 없이도 시스템 전체가 장악될 수 있음을 증명했다.

방어자는 **"이미 침해당했다(Assume Breach)"**는 가정 하에 경계 방어뿐만 아니라, 침투 후 **내부 확산을 저지**하고, **피해를 국지화**하며, **신속하게 탐지하고 대응**할 수 있는 회복탄력성 중심의 보안 전략으로 전환해야 한다.