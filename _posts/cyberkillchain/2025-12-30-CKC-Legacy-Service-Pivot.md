...
### **0.1. 작전 배경 및 개요 (Operation Background)**

본 공격 시나리오(Operation Name: **Legacy Service Pivot: Internal Pivot**)는 외부 경계 방어선이 **패치되지 않은 레거시 미들웨어(JBoss/Wildfly)**의 취약점으로 인해 돌파당했을 때, 공격자가 어떻게 내부망의 **느슨한 신뢰 관계**와 **서비스 설정 오류**를 발판 삼아 기업의 핵심 자산까지 도달하는지를 증명하는 시나리오다.

공격은 구버전 `jboss/wildfly:10.1.0.Final` 컨테이너의 원격 코드 실행(RCE) 취약점을 통해 시작된다. 공격자는 이를 통해 확보한 웹 서버의 제한된 거점을 **내부망 공격의 전초기지(Beachhead)**로 활용하여, 방화벽 뒤에 숨겨진 DB 서버의 Redis 서비스를 공격하고, 최종적으로 시스템 최고 권한(`root`)을 탈취하여 네트워크를 완전히 장악한다.

### **0.2. 수행 목적 (Objectives)**

본 시나리오는 다음의 핵심 보안 가설을 검증한다.

1.  **레거시 시스템 위험도 실증:** 지원이 종료(EOL)되었거나 패치가 누락된 레거시 미들웨어가 어떻게 현대적인 클라우드 환경의 가장 취약한 침투 경로가 되는가?
2.  **내부망 세그멘테이션 유효성 검증:** DMZ와 내부망(Private) 사이에 NSG 규칙이 존재함에도 불구하고, 허용된 애플리케이션 포트(`6379`)를 통한 수평 이동(Lateral Movement) 공격을 방어할 수 있는가?
3.  **서비스 하드닝(Hardening) 부재의 파급력 분석:** 인증 없이 외부에 노출된 단일 내부 서비스(Redis)의 설정 오류가 어떻게 전체 시스템의 **최고 권한 탈취(Root Compromise)**라는 최악의 결과로 이어지는가?

### **0.3. 타겟 시스템 아키텍처 및 공격 경로 (Architecture & Attack Vector)**

본 작전은 DMZ와 Private Subnet으로 구성된 `CKCProject`의 표준 다층 아키텍처를 대상으로 하며, 각 티어의 구성은 아래와 같다.

*   **Web Tier (DMZ Subnet `10.42.2.0/24`):** 외부 Load Balancer를 통해 8082 포트가 공개된 웹 서버 VMSS가 위치한다. 이 VM 위에서 취약한 `jboss-rce` Docker 컨테이너가 실행 중이다.
*   **DB Tier (Private Subnet `10.42.3.0/24`):** 인터넷에서 직접 접근이 불가능한 내부망에 핵심 DB 서버가 위치하며, 인증이 비활성화된 Redis 서비스가 실행 중이다.

**그림 0-3-1: 타겟 시스템 네트워크 토폴로지**
...
### **0.4. 핵심 발견사항 및 비즈니스 영향 (Key Findings & Business Impact)**

| 핵심 발견사항 (Critical Finding) | 근본 원인 (Root Cause) | 비즈니스 영향 (Business Impact) |
| :--- | :--- | :--- |
...
### **0.5. 공격 도구 및 TTPs (Tools, Tactics, and Procedures)**
...
**목표:** 타겟 인프라의 외부 접점인 Load Balancer를 대상으로 공격 표면을 매핑하고, 배포된 서비스 중 인증 없이 즉각적인 원격 코드 실행(RCE)이 가능한 **JBoss/Wildfly 취약점**을 식별하여 초기 침투 경로를 확보한다.
...
Terraform 코드(`main.tf`) 분석 결과, Load Balancer가 8082 포트를 웹 서버 VMSS로 전달하도록 설정되어 있음을 사전에 인지했다. 이 정보를 바탕으로, 해당 포트에 어떤 서비스가 실행 중인지 정밀하게 식별하기 위해 `nmap`의 서비스 버전 탐지(-sV) 스캔을 수행했다.
...
식별된 `WildFly 10.1.0.Final`은 2016년에 출시된 구버전으로, 심각한 **Java 역직렬화(Deserialization) RCE 취약점(CVE-2017-12149)**에 노출되어 있을 가능성이 매우 높다. 공격자는 이 가설을 검증하기 위해, 해당 취약점의 트리거가 되는 특정 엔드포인트의 존재 여부를 확인했다.
...
*   **[T1608.002] Stage Capabilities: Upload Tool:** 페이로드 배포를 위한 스테이징(Staging) 서버 구축.
...
### **2.2. 페이로드 배포 인프라(Staging) 구축**


Java 역직렬화 공격 시, 수 메가바이트(MB)에 달하는 C2 임플란트 전체를 HTTP 요청 Body에 담아 보내는 것은 비효율적이고 실패 확률이 높다. 따라서 공격자는 **"다운로더(Downloader)"** 방식의 2단계 공격을 설계했다.
...
JBoss 서버가 페이로드를 역직렬화하는 순간, 시스템 명령어를 실행하도록 유도하는 "탄두(Warhead)"를 제작했다. 이를 위해 **`ysoserial`** 도구와 `WildFly 10`에 포함된 `Apache Commons Collections` 라이브러리를 악용하는 **`CommonsCollections5`** 가젯 체인을 사용했다.

```bash
# ysoserial을 사용하여 악성 직렬화 객체(payload.ser) 생성
# Target Command: C2 스테이징 서버에서 임플란트를 다운로드하여 실행
...
**Gadget Chain 선정**
`ysoserial`은 다수의 가젯 체인을 제공했다. 공격자는 타겟인 `WildFly 10.1.0.Final`의 클래스패스(Classpath)에 **`Apache Commons Collections`** 라이브러리가 기본적으로 포함되어 있음을 사전에 파악했다. 따라서 해당 라이브러리의 취약한 클래스를 악용하는 **`CommonsCollections5`** 가젯 체인을 선택했다. 이는 타겟 환경에 대한 정확한 이해를 바탕으로 익스플로잇 성공률을 극대화하기 위한 전술적 결정이었다.
...
```bash
# Attacker's Staging Server Logs (Python HTTP)
...
*   **아웃바운드 트래픽 모니터링 (Egress Filtering):**
    *   **Anomaly:** 웹 서버(DMZ)가 업무와 무관한 외부의 낯선 IP/포트(예: Attacker Staging Server)로 실행 파일(.elf)을 다운로드하는 행위는 명백한 침해 지표(IoC)이다. 모든 비인가 아웃바운드 연결은 기본적으로 차단(Default Deny)되어야 한다.

---

## **4. 악용 (Exploitation)**

**목표:** JBoss 컨테이너 내부에서 실행된 C2 임플란트를 통해 공격자 서버로 **콜백(Callback)** 연결을 수립하고, **초기 침투(Initial Access)**를 완료하여 내부망 공격을 위한 첫 번째 거점을 확보한다.
...
**권한 분석**
*   **User:** 현재 권한은 컨테이너 내부의 **`root`**이다.
*   **Limitation:** 컨테이너 내부에서는 최고 권한이지만, 호스트 VM에 대한 직접적인 제어 권한은 없으며 컨테이너 내부에 격리되어 있다.
*   **Next Step:** 따라서 다음 목표는 이 컨테이너를 **내부망 스캐닝을 위한 피봇 포인트(Pivot Point)**로 활용하여, 네트워크적으로 접근 가능한 다른 취약한 서비스를 찾아 수평 이동(Lateral Movement)하는 것으로 설정되었다.

### **4.3. 방어 회피 분석 (Defense Evasion Analysis)**
...
**목표:** 초기 침투로 확보한 컨테이너 내부의 C2 세션은 컨테이너가 재시작되거나 삭제되면 소멸된다. 공격자는 현재 확보한 **`jboss` 사용자 권한 범위 내**에서 생존 가능성을 높이고, 후속 공격을 안정적으로 수행하기 위한 **사용자 레벨 지속성(User-Land Persistence)** 메커니즘을 구축한다.
...
### **5.1. 아티팩트 은닉 및 위장 (Artifact Hiding & Masquerading)**
...
```bash
# Sliver Session (Interactive Shell)
...
추가적으로, `touch` 명령어를 사용하여 악성 파일의 생성/수정 시간(MAC Time)을 시스템의 정상 파일(`/bin/ls`)과 동일하게 변조했다. 이는 포렌식 분석가가 "최근 생성된 파일"을 기준으로 타임라인 분석을 수행할 때 탐지를 회피하기 위한 **안티 포렌식(Anti-Forensics)** 기법이다.
...
![Timestomping](/assets/images/ckc/c/c.5.2.timestomping.png)

**안티 포렌식 기법 'Timestomping'**
`touch -r /bin/ls ...` 명령어는 고도로 계산된 **안티 포렌식(Anti-Forensics)** 기법이다. 공격자는 악성 파일의 시간을 시스템 핵심 바이너리의 시간으로 위조함으로써, 타임라인 분석에서 자신의 흔적을 숨기고 초기 침투 시점을 위장하여 조사관에게 혼란을 준다.

### **5.2. 사용자 레벨 지속성 확보 (User-Land Persistence)**
...
### **6.2. 하트비트 설정 및 지터 적용 (Heartbeat & Jitter)**
...
![C2 Reconfig](/assets/images/ckc/c/c.6.1.reconfig-jitter.png)

### **6.3. 연결 상태 검증 (Connection Verification on Victim)**

공격자는 확보한 C2 셸을 통해 타겟 컨테이너 내부의 네트워크 연결 상태를 확인하여, 백도어가 정상적으로 동작하고 아웃바운드 방화벽을 우회했음을 검증했다.

```bash
# Victim Side Shell Command
...
*   **네트워크 트래픽 분석 (NTA/NDR):**
    ...
    *   **JA3/JA3S Fingerprinting:** 일반적인 웹 브라우저나 Java 클라이언트가 아닌, Sliver/Go 언어 특유의 TLS 핸드셰이크 지문(Fingerprint)을 식별하여 차단해야 한다.
*   **아웃바운드 통제 (Egress Filtering):** **가장 효과적인 방어책**으로, 서버 팜에서는 승인된 도메인(Allowlist) 외의 모든 아웃바운드 연결을 기본적으로 차단(Default Deny)해야 한다.

---

## **7. 목적 달성 (Action on Objectives)**

**목표:** 확보된 웹 서버 컨테이너(DMZ)를 내부망 공격의 발판으로 삼아 내부 DB 서버를 탐색하고, **Redis 서비스의 설정 오류**를 악용하여 시스템의 **`root` 권한을 획득**함으로써 네트워크를 장악한다.
...
### **7.1. 1단계: 내부 정찰 및 타겟 식별 (Internal Reconnaissance)**
...
### **7.2. 2단계: Redis 악용 및 SSH 키 주입 (Redis Abuse & SSH Key Injection)**
...
### **7.3. 3단계: 수평 이동 및 DB 서버 완전 장악 (Lateral Movement & Complete Compromise)**
...
### **7.3.1. 중요 데이터 식별 및 유출 (Data Exfiltration)**

DB 서버를 장악한 공격자는 Redis에 평문으로 저장된 DB 자격 증명을 발견하고, 이를 이용해 실제 고객 데이터를 덤프(Dump)하여 외부로 유출했다.

**1. DB 자격 증명 발견 (Redis Inspection)**
Redis 키를 조사하던 중, 개발자가 실수로 남겨둔 DB 접속 정보를 발견했다.

![Redis 등 DB 접속 정보 발견](/assets/images/ckc/c/c.7.11.redis-db-creds.png)

**2. 데이터 덤프 (Data Staging)**
...
**4. 데이터 유출 확인 (Exfiltration Proof)**
...
*   **네트워크 세그멘테이션 (Micro-Segmentation):** **가장 중요한 방어책**으로, 웹 서버 서브넷(`10.42.2.0/24`)에서 DB 서버의 관리 포트(`6379`, `22`)로 향하는 트래픽은 NSG/Firewall에서 원천적으로 **Deny** 처리되어야 한다. 웹 서버는 오직 DB 포트(`3306`)에만 접근할 수 있어야 한다.
*   **Redis 보안 강화 (Hardening):**
...
본 공격 시나리오는 **"외부의 작은 틈(Legacy Vulnerability)이 내부의 구조적 결함과 만나면 시스템 전체 붕괴로 이어진다"**는 보안의 나비 효과를 실증하였다. 공격자는 고난이도의 제로데이 공격이 아닌, 관리되지 않은 **구형 미들웨어**와 보안 설정이 누락된 **내부 서비스**를 연계하여 최고 권한을 탈취했다.
...
### **8.2. 보안 강화 로드맵 (Strategic Roadmap)**

| 단계 | 목표 | Action Items |
| :--- | :--- | :--- |
| **Phase 1: 긴급 조치 (24h)** | **현재 위협 제거 및 출혈 차단** | 1. **JBoss/WildFly 격리 및 패치:** 해당 서버를 외부망에서 즉시 분리하거나 최신 버전으로 업그레이드. 불가능할 경우 WAF로 `/invoker/*` 경로 접근 차단. <br> 2. **Redis 보안 하드닝:** `bind 127.0.0.1`, `requirepass` 설정, `protected-mode yes` 적용. <br> 3. **SSH 키 전수 조사:** 모든 서버의 `authorized_keys` 파일을 점검하여 비인가 키 삭제. |
| **Phase 2: 구조 개선 (1 Month)** | **내부 확산 방지 및 권한 최소화** | 1. **마이크로 세그멘테이션:** NSG 규칙을 강화하여, 웹 서브넷에서 DB 서브넷으로 오직 필요한 포트(`3306`)만 허용하고 나머지는 명시적으로 **Deny**. <br> 2. **프로세스 권한 분리:** 모든 애플리케이션 데몬(Redis, JBoss 등)을 **Non-Root 전용 계정**으로 구동하도록 `systemd` 서비스 파일 수정. <br> 3. **위험한 명령어 비활성화:** `redis.conf`에서 `rename-command CONFIG ""` 설정을 통해 런타임 중 설정 변경 기능 원천 차단. |
| **Phase 3: 체질 개선 (6 Months)** | **침해 탐지 능력 확보 및 예방 체계 구축** | 1. **내부망 트래픽 분석(NDR):** Zeek, Suricata 등을 도입하여 내부 서브넷 간의 비정상적인 통신(예: 웹 서버 -> DB 서버의 Redis 포트 접속)을 실시간으로 탐지. <br> 2. **정기적 취약점 관리:** 정기적인 취약점 스캔 및 모의 해킹을 제도화하여 EOL 자산을 식별하고 폐기하는 프로세스 수립. <br> 3. **보안 구성 관리 자동화:** Ansible, Chef 등을 사용하여 모든 서버에 일관된 보안 설정(Hardening)을 강제하고, 설정 변경을 추적 및 감사. |

### **8.3. 기술적 세부 권고 (Technical Deep Dive)**
...
Legacy Service Pivot 시나리오는 **"오래된 기술 부채(Legacy Tech Debt)"**가 어떻게 보안 사고의 도화선이 되는지를 명확히 보여주었다. 외부의 낡은 문(JBoss)을 통해 들어온 공격자는 내부의 허술한 관리(Redis Root) 덕분에 왕좌(Root)를 차지할 수 있었다.

방어자는 **"경계가 뚫리는 것은 시간문제(Assume Breach)"**임을 인정해야 한다. 따라서 침투를 100% 막는 것보다, 침투한 공격자가 내부에서 자유롭게 이동하지 못하도록 **네트워크를 구획화(Segmentation)**하고, 권한을 잘게 쪼개어 **피해를 국지화(Blast Radius Reduction)**하는 **Zero Trust 아키텍처**로의 전환이 시급하다.
