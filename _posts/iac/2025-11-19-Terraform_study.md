...
*   **프로비저닝(Provisioning)**: 클라우드 환경이나 자체 데이터센터에서 정책에 맞게 필요한 IT 자원(VM, 컨테이너, 네트워크, 스토리지 등)을 할당, 배치, 배포하여 사용할 수 있는 상태로 준비하는 과정이다.
*   **구성 관리(Configuration Management)**: 프로비저닝된 인프라 내부의 OS 설정, 패키지 설치, 애플리케이션 환경 등을 변경하고 상태를 일정하게 유지하는 과정이다.
