...
* **프로비저닝(Provisioning)**: 서버, 네트워크, DB 등 인프라 자원 자체를 생성하고 할당하는 작업. (Terraform의 주 역할)
* **구성 관리(Configuration Management)**: 이미 생성된 인프라 내부에 패키지를 설치하고, OS나 애플리케이션의 설정 파일을 일관되게 배포 및 유지하는 작업. (Ansible의 주 역할)
