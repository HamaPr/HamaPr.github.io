...
이번에는 파일을 만들어서 서버(`master` 원격 저장소)로 **Push** 해야 했습니다. 하지만 `.gitignore` 파일이 작업을 방해했습니다. 이를 우회해서 파일을 강제로 올리는 법을 배웠습니다.
...
| 명령어 | 설명 |
|--------|------|
| `git add -f [파일명]` | `.gitignore`에 의해 무시되는 파일을 **강제로** 스테이징 영역에 추가 |
