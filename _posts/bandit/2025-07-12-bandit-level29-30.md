...
이번에도 비밀번호가 안 보였습니다. `git log`를 뒤져봐도 비밀번호를 지운 흔적이 없습니다. 그렇다면 비장의 무기, **다른 브랜치(Branch)** 에 숨겨져 있을 가능성이 높다고 판단했습니다. 개발자들은 보통 `master` 브랜치가 아닌 `dev`나 `feature` 브랜치에서 작업하니까요.
...
| 명령어 | 설명 |
|--------|------|
| `git branch -a` | 로컬과 원격(Remote)의 모든 브랜치 목록을 보여줌 |
| `git checkout [브랜치명]` | 해당 브랜치로 전환(Switch) |
