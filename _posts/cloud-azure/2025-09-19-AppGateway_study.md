...
### URL 경로 기반 라우팅 및 다중 사이트 호스팅
 Application Gateway는 단일 IP로 여러 웹사이트(다중 사이트 호스팅)를 서비스할 수 있으며, 요청 URL의 경로(`/api/*`, `/images/*`)에 따라 서로 다른 백엔드 풀로 트래픽을 분배(URL 라우팅)할 수 있다. 마이크로서비스 아키텍처나 여러 도메인을 하나의 게이트웨이로 통합 관리할 때 필수적인 기능이다.
