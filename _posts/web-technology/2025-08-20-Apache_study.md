...
Apache의 동작은 설정 파일 `.conf` 안의 지시어를 통해 제어된다.
...
Apache는 특정 디렉터리나 파일에 대한 접근을 IP 주소나 사용자 인증을 통해 제한할 수 있다.

### IP 기반 접근 제어

특정 IP만 허용하거나 차단하는 설정이다.
...
#### 4. 접속 테스트
브라우저에서 해당 URL 접속 시 인증 팝업이 표시된다.
