...
**MySQL**은 전 세계적으로 가장 널리 사용되는 오픈소스 관계형 데이터베이스 관리 시스템(RDBMS)이다.
웹 애플리케이션의 핵심 데이터를 저장하므로, MySQL의 보안 설정과 올바른 쿼리 작성은 정보 유출을 막는 최후의 보루와 같다.
...
### 2.2. 데이터 조회 (DVWA 예시)
DVWA의 `users` 테이블 구조를 파악하고 데이터를 조회해 본다.
...
보안을 위해 애플리케이션은 `root` 계정을 사용해서는 안 되며, 최소한의 권한만 가진 전용 계정을 사용해야 한다.
...
`dvwa` 데이터베이스에 대해서만 `SELECT`, `INSERT`, `UPDATE` 권한을 부여한다. (`DROP`이나 `GRANT` 권한은 제외)
```sql
...
SQL Injection은 사용자 입력값이 쿼리의 일부분으로 해석되어 발생한다. 이를 막기 위해 **Prepared Statement(준비된 구문)**를 사용해야 한다.
...
```
이렇게 하면 입력값은 오직 '데이터'로만 처리되어 SQL 문법에 영향을 줄 수 없다.
...
3.  **파일 권한**: DB 데이터 파일이 저장된 디렉터리(`/var/lib/mysql`)의 권한을 엄격하게 관리한다.
