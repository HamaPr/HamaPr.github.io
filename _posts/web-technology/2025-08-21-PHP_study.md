...
*   **파일 업로드 검증**: 업로드된 파일의 확장자뿐만 아니라 MIME 타입과 매직 바이트를 함께 검증하여 웹쉘 업로드를 방지한다.
*   **함수 비활성화**: `php.ini`의 `disable_functions` 설정을 통해 `system`, `exec`, `shell_exec`, `passthru` 등 OS 명령어 실행 함수를 비활성화한다.
*   **최신 버전 유지**: PHP 버전을 최신 상태로 유지하여 알려진 취약점에 대한 패치를 적용한다.
