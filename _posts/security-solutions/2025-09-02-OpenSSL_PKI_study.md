...
*   **개인키 보호**: CA 개인키(`ca.key`)는 반드시 암호화하고 접근 권한(400)을 제한한다. 유출 시 전체 PKI 신뢰가 붕괴된다.
*   **강력한 알고리즘**: RSA는 최소 2048비트(권장 4096), 해시는 SHA-256 이상을 사용한다. SHA-1, MD5는 사용 금지.
*   **인증서 만료 관리**: 인증서 만료 알림 시스템을 구축하여 갱신 시점을 놓치지 않도록 한다.
*   **CRL/OCSP 구성**: 인증서 폐기 관리를 위해 CRL(인증서 폐기 목록) 또는 OCSP(온라인 상태 검증)를 구성한다.
