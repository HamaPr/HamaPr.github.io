...
*   **LACP 모드 협상 주의**: `Static (On)` 모드는 프로토콜 검증 없이 강제 결합하므로, 잘못된 케이블 연결 시 루프가 발생할 수 있다. 반드시 `Active-Active` 또는 `Active-Passive` 조합을 사용한다.
*   **BPDU Guard 연동**: EtherChannel 포트에도 BPDU Guard를 설정하여 예상치 못한 스위치 연결을 감지한다.
*   **MAC Flooding 공격 주의**: 채널 멤버 포트가 MAC 주소 테이블을 넘치게 하는 공격을 받으면 전체 채널이 영향을 받을 수 있다. Port Security를 추가로 적용한다.
*   **로드 밸런싱 알고리즘 점검**: 의도치 않게 한 링크로만 트래픽이 쏠리면 대역폭 낭비와 함께 해당 링크 과부하로 장애가 발생할 수 있다.
