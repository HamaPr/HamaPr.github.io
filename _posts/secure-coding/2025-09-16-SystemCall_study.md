...
*   **경로 검증**: 사용자 입력을 파일 경로로 사용할 때 디렉터리 트래버설(`../`)을 방지하여 임의 파일 접근을 차단한다.
*   **최소 권한 원칙**: 파일을 열 때 필요한 최소한의 권한만 부여한다. (`O_RDONLY` 우선)
*   **Race Condition 주의(TOCTOU)**: 파일 존재 확인 후 열기까지 사이에 공격자가 심볼릭 링크를 조작할 수 있으므로, `open(O_CREAT|O_EXCL)` 등 원자적 연산을 사용한다.
*   **umask 설정**: 파일 생성 시 권한을 명시적으로 지정하고, 민감 파일은 `0600` 이하로 제한한다.
