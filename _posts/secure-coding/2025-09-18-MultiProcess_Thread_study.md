...
*   **Race Condition 방지**: 공유 자원 접근 시 반드시 Mutex나 Semaphore를 사용하여 데이터 무결성을 보장한다.
*   **자식 프로세스 권한**: `fork()` 후 권한 상승이 필요한 경우 `setuid`를 신중하게 사용하고, 불필요한 권한은 즉시 드롭한다.
*   **IPC 보안**: 공유 메모리나 파이프를 통한 프로세스 간 통신 시 접근 권한을 제한하고, 입력 데이터를 검증한다.
*   **시그널 핸들러 주의**: 시그널 핸들러 내에서는 비동기 안전(Async-Signal-Safe)한 함수만 호출해야 한다.
