...
한국방송통신전파진흥원(KCA)에서 시행하는 정보보안기사 1차 필기시험에 합격했다. 정보보안기사는 네트워크와 시스템의 보안 취약점을 분석하고, 침해사고에 대응하며, 보안 정책을 수립하는 전문 인력을 양성하기 위한 국가기술자격이다.

본 시험은 시스템 보안, 네트워크 보안, 애플리케이션 보안, 정보보안 일반, 정보보안 관리 및 법규의 총 5과목으로 구성되어 있다.
...
| 키워드 | 설명 |
|--------|------|
| `lsof` | 열린 파일 및 네트워크 연결 확인 (List Open Files) |
| `lastb` | 로그인 실패 기록 확인 (`/var/log/btmp`) |
| `find / -perm -4000` | SUID 설정 파일 검색 |
| `/etc/shadow` | 암호화된 패스워드 해시 저장 파일 |
| Canary | 스택 버퍼 오버플로우 탐지용 가드 값 |
| ASLR | 주소 공간 배치 난수화 (Address Space Layout Randomization) |
| DEP/NX | 데이터 실행 방지 (Data Execution Prevention) |
...
| AH (Authentication Header) | IPsec 인증 프로토콜 (무결성 보장) |
| ESP (Encapsulating Security Payload) | IPsec 암호화 프로토콜 (기밀성 + 무결성) |
| NAC | 네트워크 접근 제어 (Network Access Control) |
| 3-way handshake | TCP 연결 수립: SYN → SYN+ACK → ACK |

### 애플리케이션 보안

| 키워드 | 설명 |
|--------|------|
| Prepared Statement | SQL Injection 방어를 위한 파라미터화 쿼리 |
| HttpOnly | JavaScript로 쿠키 접근 차단 |
| SameSite | CSRF 방어를 위한 쿠키 속성 |
| CSP | 콘텐츠 보안 정책 (Content Security Policy) |
| `strncpy` | `strcpy` 대신 사용하는 안전한 문자열 복사 함수 |

### 암호학

| 키워드 | 설명 |
|--------|------|
| HMAC | 해시 + 대칭키 결합 (인증, 무결성) |
| OCSP | 인증서 유효성 실시간 조회 |
| CRL | 인증서 폐기 목록 (비실시간) |
| Diffie-Hellman | 키 교환 알고리즘 |
| bcrypt/scrypt/Argon2 | 패스워드 해싱 권장 알고리즘 |
...
**네트워크 공격 식별**
- 패킷 덤프를 보고 **SYN Flooding**, **ARP Spoofing**, **DNS Amplification** 공격 유형 식별
- iptables 룰 작성 문제
...
| 모델 | 권한 결정자 | 특징 | 예시 |
|------|------------|------|------|
| DAC | 소유자 | 임의적, 유연함 | Linux 파일 권한 |
| MAC | 시스템 | 강제적, 엄격함 | SELinux, 군사 시스템 |
| RBAC | 역할 기반 | 관리 용이 | 기업 권한 관리 |
...
| 솔루션 | 계층 | 역할 |
|--------|------|------|
| 방화벽 | L3-L4 | IP/Port 기반 접근 제어 |
| IDS/IPS | L3-L7 | 침입 탐지/차단 |
| WAF | L7 | 웹 애플리케이션 공격 방어 |
| NAC | L2 | 단말 보안 상태 검사 |
| DLP | L7 | 데이터 유출 방지 |

---

정보보안기사 필기 학습을 통해 보안의 기본 개념부터 암호학, 법규까지 전반적인 지식 체계를 정립할 수 있었다.
