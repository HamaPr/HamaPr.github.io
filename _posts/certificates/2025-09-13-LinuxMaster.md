...
리눅스 마스터 2급 자격증을 취득했다. 한국정보통신진흥협회(KAIT)에서 주관하는 국가공인 민간자격으로, 리눅스 기반 서버 및 시스템 관리 능력을 인증하는 자격증이다.

이 과정을 통해 파일 시스템, 사용자 관리, 네트워크 설정까지 리눅스 운영 및 관리 기술을 체계적으로 학습했다.
...
| 키워드 | 설명 |
|--------|------|
| `ps aux` | BSD 형식 프로세스 전체 출력 |
| `ps -ef` | System V 형식 프로세스 출력 |
| `kill -9` | SIGKILL (강제 종료) |
| `systemctl enable` | 부팅 시 자동 시작 등록 |
| `crontab -e` | 사용자 cron 작업 편집 |

### 네트워크

| 키워드 | 설명 |
|--------|------|
| `ip addr show` | IP 주소 확인 (ifconfig 대체) |
| `netstat -anp` | 포트 및 연결 상태 확인 |
| `ping` | ICMP 연결 테스트 |
| `traceroute` | 경로 추적 |
...
```bash
useradd -d /home/user1 -s /bin/bash user1  # 홈 디렉터리, 셸 지정
passwd user1                                # 비밀번호 설정
usermod -aG wheel user1                     # 그룹 추가
userdel -r user1                            # 홈 디렉터리 포함 삭제
```
...
mkfs.ext4 /dev/sdb1  # 파일시스템 포맷
mount /dev/sdb1 /mnt # 마운트
# /etc/fstab에 추가하면 영구 마운트
```
...
| 셸 | 설명 |
|----|------|
| bash | Bourne Again Shell, 리눅스 기본 셸 |
| sh | Bourne Shell, 유닉스 최초 셸 |
| csh | C Shell, C 언어 문법 기반 |
| ksh | Korn Shell, sh + csh 결합 |
...
|------|------------|-------------|
| 패키지 형식 | .rpm | .deb |
| 설치 명령어 | rpm -ivh | dpkg -i |
| 의존성 해결 | yum / dnf | apt-get / apt |
...
리눅스 마스터 2급 학습을 통해 리눅스 시스템 관리의 기초를 체계적으로 정립할 수 있었다.
