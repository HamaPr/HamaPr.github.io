...
*   **원본 파일 손상 주의**: `sed -i` 옵션은 원본 파일을 직접 수정한다. 실수로 중요한 파일을 손상시킬 수 있으므로 `-i.bak` 옵션으로 백업본을 먼저 생성하거나, 수정 전 결과를 미리 확인한다.
*   **신뢰할 수 없는 입력 처리**: 외부 입력(로그 파일, 사용자 데이터 등)을 awk/sed로 처리할 때, 입력 데이터에 예상치 못한 문자(예: `;`, `|`)가 포함될 수 있어 명령어 인젝션 위험이 있다. 입력 데이터를 검증하거나 필터링한다.
*   **정규 표현식 주의**: 복잡한 정규식은 ReDoS(Regular Expression Denial of Service) 공격에 취약할 수 있다. 특히 중첩된 수량자(`(a+)+`)는 피한다.
