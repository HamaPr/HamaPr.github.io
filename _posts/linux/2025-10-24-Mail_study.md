...
*   **Open Relay 차단 (필수)**: `/etc/mail/access`에서 신뢰할 수 있는 IP/도메인만 `RELAY`로 설정한다. Open Relay 상태면 스팸 메일 발송 서버로 악용된다.
*   **SPF/DKIM/DMARC 설정**: DNS에 SPF 레코드를 추가하여 발신 서버를 인증하고, 위조된 발신자를 차단한다.
*   **평문 전송 주의**: SMTP(25), POP3(110), IMAP(143)은 기본적으로 암호화되지 않는다. 가능하면 SMTPS(465), POP3S(995), IMAPS(993)을 사용한다.
*   **Dovecot 인증 보안**: 무차별 대입 공격 방지를 위해 Fail2Ban을 연동하거나 인증 시도 횟수를 제한한다.
