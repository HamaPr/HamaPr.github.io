...
*   **디스크 암호화 (LUKS)**: 민감한 데이터를 저장하는 LV는 LUKS(Linux Unified Key Setup)로 암호화하여 물리적 도난 시에도 데이터를 보호한다.
*   **물리적 보안**: LVM은 논리적 관리 도구일 뿐, 디스크에 물리적으로 접근할 수 있는 공격자는 데이터를 읽을 수 있다. 서버 물리 보안을 철저히 한다.
*   **스냅샷 관리**: 스냅샷은 공간을 소비하므로 주기적으로 정리한다. 오래된 스냅샷이 쌓이면 VG 공간 부족으로 시스템 장애가 발생할 수 있다.
