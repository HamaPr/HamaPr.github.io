...
40: 분할되어 전송된 여러 TCP 패킷을 재조립하여 사람이 읽을 수 있는 하나의 데이터 스트림(대화 내용)으로 보여준다.
...
### 샘플 PCAP 파일 다운로드
Wireshark Wiki에서 다양한 프로토콜의 샘플 캡처 파일을 제공한다.
*   [Wireshark Sample Captures](https://wiki.wireshark.org/SampleCaptures)
*   `http.cap`, `telnet.cap` 등을 다운로드하여 분석 연습 가능

### 로컬 트래픽 캡처
```bash
# 리눅스에서 tcpdump로 캡처 후 분석
sudo tcpdump -i eth0 -w practice.pcap port 80
```
...
### 탐지 방법
*   **Promiscuous Mode 탐지**: 네트워크 인터페이스가 무차별 모드로 변경되었는지 확인
*   **ARP 감시**: `arpwatch` 등을 통해 ARP Spoofing 공격 시도 탐지

### 방어 방법
*   **종단간 암호화 (E2EE)**: HTTPS, SSH, TLS 등 암호화 프로토콜을 강제하여 패킷이 탈취되어도 내용을 볼 수 없게 한다.
*   **VPN 사용**: 공용 네트워크(카페 Wi-Fi 등)에서는 VPN을 사용하여 터널링된 암호화 통신을 한다.
*   **스위치 보안**: 포트 보안(Port Security) 및 Dynamic ARP Inspection(DAI) 설정
